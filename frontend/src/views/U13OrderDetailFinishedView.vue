...
import { submitOrderReview } from "../api/runner";
...
const loadOrder = async () => {
  const orderId = appStore.activeOrderId;
...
<template>
  <MobileFrame>
    <div class="page">
          ...
          <el-card class="status-card" shadow="never">
            <div class="status-top">
              <div>
                ...
                <div class="status-desc">订单已送达，感谢使用校园跑腿服务</div>
              </div>
              <el-tag effect="dark" type="success" round>已完成</el-tag>
            </div>
            <el-divider />
            <div class="meta-row"><span class="k">订单号</span><span class="v mono">{{ order.id }}</span></div>
            <div class="meta-row"><span class="k">服务类型</span><span class="v">{{ order.serviceType }}</span></div>
          </el-card>

          <el-card class="info-card" shadow="never">
            <div class="card-title">履约信息</div>
            <div class="route"><div class="dot from"></div><div class="route-main"><div class="place">{{ order.from }}</div><div class="hint">起点</div></div></div>
            <div class="route"><div class="dot to"></div><div class="route-main"><div class="place">{{ order.to }}</div><div class="hint">终点</div></div></div>
            <el-divider />
            <div class="meta-row"><span class="k">联系电话</span><span class="v">{{ order.phoneMasked }}</span></div>
          </el-card>

          <el-card class="fee-card" shadow="never">
            <div class="card-title">费用明细</div>
            <div class="total"><span>实付</span><strong>¥{{ order.total.toFixed(2) }}</strong></div>
          </el-card>

          <el-card class="fee-card" shadow="never">
            <div class="card-title">订单评价</div>
            <div class="meta-row">
              <span class="k">评分（1-5）</span>
              <el-input v-model.number="reviewRating" type="number" :min="1" :max="5" />
            </div>
            <el-input v-model="reviewComment" type="textarea" :rows="3" maxlength="120" placeholder="填写对跑腿员的评价（可选）" />
            <el-button class="primary" style="margin-top: 10px" :disabled="submitted" @click="submitReview">
              {{ submitted ? "已评价" : "提交评价" }}
            </el-button>
          </el-card>
        </template>
      ...
      <footer class="footer">
        <el-button class="primary" type="primary" size="large" round @click="emit('again')">再来一单</el-button>
...
.meta-row .k { color:rgba(167,186,227,.85); font-size:12px; white-space:nowrap; }
